    ...
    <div class="container">
      <div class="row">
        ...
        <main class="posts-listing col-lg-8"> 
          <div class="container">
          <h3>REZULTATI ZA POJAM: "${search }"</h3>
            <div class="row">
              <!-- post -->
...
<%--               <c:if test="${fn:length(postList) > 0}"> --%>
              <div class="post col-xl-6">
                <div class="post-thumbnail"><a href="blog-post-page?id=${postList[i].id }&title=${postList[i].title}"><img src="${pageContext.request.contextPath}${postList[i].image }" alt="..." class="post-image"></a></div>
                <div class="post-details">
                  <div class="post-meta d-flex justify-content-between">
                    <div class="date meta-last">${postList[i].date }</div>
                    <c:if test="${postList[i].category.name!='Uncategorized' }">
                    <div class="category"><a href="blog-category-page?id=${postList[i].category.id }&name=${postList[i].category.name}">${postList[i].category.name }</a>
                    	| <a href="blog-subcategory-page?id=${postList[i].subcategory.id }&name=${postList[i].subcategory.name}">${postList[i].subcategory.name }</a>
                    </div>
                  	</c:if>
                  	<c:if test="${postList[i].category.name=='Uncategorized' }">
                    <div class="category">${postList[i].category.name }</div>
                  	</c:if>
                  ...
                  <footer class="post-footer d-flex align-items-center"><a href="blog-author-page?username=${postList[i].author.username }" class="author d-flex align-items-center flex-wrap">
                      <div class="avatar"><img src="${pageContext.request.contextPath}${postList[i].author.image }" alt="..." class="img-fluid"></div>
              ...
              <c:if test="${postList.size() == 0 }">
              	<h2 class="not-news">Nema vesti za traženi pojam</h2>
              </c:if>
            </div>
          ...
          <div class="widget search">
            ...
            <form action="blog-search-page">
                  <div class="form-group">
                    <input type="search" name="search" id="search" placeholder="Unesite pojam za pretragu">
          ...
          <div class="widget latest-posts">
            ...
            <div class="blog-posts"><a href="blog-post-page?id=${lastPost.id }&title=${lastPost.title}">
              <div class="item d-flex align-items-center">
                <div class="image"><img src="${pageContext.request.contextPath }${lastPost.image }" alt="..." class="img-fluid"></div>
